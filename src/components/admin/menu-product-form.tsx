...
export function MenuProductForm({ categories, locations }: { categories: MenuCategoryDto[]; locations: LocationOption[] }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(typeof json === "object" && json && "error" in json && typeof json.error === "string" ? json.error : "The product could not be created.");
  ...
  return <motion.form onSubmit={submit} className="mt-7 space-y-6" initial={reduceMotion ? false : { opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
    <fieldset disabled={busy} className="cb-panel grid gap-5 p-5 sm:grid-cols-2 sm:p-7">
      ...
      <Field label="Product image (JPEG, PNG, or WebP; max 5 MB)" wide><input name="image" type="file" accept="image/jpeg,image/png,image/webp" className="cb-field" /></Field>
    </fieldset>
    <fieldset disabled={busy} className="cb-panel p-5 sm:p-7"><legend className="px-2 font-semibold text-zinc-100">Location offers</legend><p className="mb-5 text-sm text-zinc-500">Select only locations that sell this product. Prices are stored as exact decimals.</p><div className="space-y-3">{locations.map((location) => <div key={location.id} className="grid items-center gap-3 rounded-xl border border-zinc-800 p-4 sm:grid-cols-[1fr_9rem_6rem_auto]"><label className="flex items-center gap-3 text-sm font-medium text-zinc-200"><input type="checkbox" checked={Boolean(selected[location.id])} onChange={(event) => setSelected((current) => ({ ...current, [location.id]: event.target.checked }))} />{location.name}</label><input aria-label={`${location.name} price`} name={`price-${location.id}`} className="cb-field" defaultValue="0.00" inputMode="decimal" disabled={!selected[location.id]} required={selected[location.id]} /><input aria-label={`${location.name} currency`} name={`currency-${location.id}`} className="cb-field uppercase" defaultValue="USD" maxLength={3} disabled={!selected[location.id]} required={selected[location.id]} /><label className="flex items-center gap-2 text-sm text-zinc-400"><input name={`available-${location.id}`} type="checkbox" defaultChecked disabled={!selected[location.id]} /> Available</label></div>)}</div></fieldset>
